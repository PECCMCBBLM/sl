...
$(document).ready(function () {
    //send_command("1|ready");  //Don't think we need this - just saying the hud has loaded

    //Populate the HTML in the hud with values that would take up too many kb in the script
    $('.material-icons').addClass('notranslate');
    $('#pbHdr').html("Please sit on the ball when it reaches you.");
    $('.i1').html("Ask if you can synchronize your dances with this person. If they accept, they will control your dancing.");
    $('.i2').html("Click a name to invite this person to dance.");
    $('.i3').html("Ask selected person to dance a couple dance with you.");
    $('.i4').html("Click a name to remove this person from the dance machine.");
    $('.i5').html("Sets how often dances will change when dancing from the queue, or when dancing random dances.");
    $('.i6').html("If you have a small or large avatar, use this control to adjust your position above the floor when sat on dance poseballs. Note this setting will persist across the grid.<br />To adjust your height temporarily for a particular dance, please use <b>Page-Up/Page-Down</b> buttons whilst seated.");
    $('.i7').html("If enabled, hosts can control your single dances, allowing them to lead a group synchronised dance.");
    $('.i8').html("Take this action when someone asks to sync their dancing with you.");
    $('.i9').html("Take this action when someone asks to dance with you.");
    $('.i10').html("Your favourite dances are synchronised across the grid. Favourites you select whilst using this Omnio will appear as favourites on all other Omnios in world.");
    $('.i11').html("To configure the Omnio, click the button above, and use the drop down menu when it appears. ");
                ...
                duration: 500, complete: function () {
                    ...
                    if ((newChoice == 1) || (newChoice == 3) || ($('body').data('h') == true)) { //Single or Multiple Dance Choice
                        ...
                        if (newChoice == 2) { //Couples Dance
                            send_command("4196353|2|MF"); //4194304|2048|1
                        }
                        else { //Line Dance
                            send_command("4196353|1|-"); //4194304|2048|1
                        }
                        $('#pbWait').show();
                    }
                }
    ...
    $('#rCancel').on("click", function () {  //Cancel Poseball Request
        send_command("4196354|-1");  //4194304|2048|2
        $('#pbWait').hide();
        showDC();
    });
    ...
    $('.ctl_stop').on("click", function () {
        //Save settings and send stop command
        ...
        $.ajax({
            type: "PUT",
            url: encodeURI("https://interfaces.thereinspirit.com/api/setting/setsettings/051bc4c1-5188-4002-985f-e8e0e77da127/Actingill Igaly/-1"),
            dataType: "json",
            data: settingdata,
            fail: function (jqXHR, textStatus, errorThrown) {
                alert(errorThrown);
            },
            complete: send_command('4196352|stop') //4096304|2048
        });
    });
    ...
    $('#ctl_play').on("click", function () {
        if (!$(this).parent().hasClass('dis')) {
            if ($(this).text() == 'play_circle') {
                ...
                send_command('4194817|stand'); //4094304|512|1
            }
        }
    });
    ...
    $.ajax({
        type: "PUT",
        url: encodeURI("https://interfaces.thereinspirit.com/api/setting/getsettings/051bc4c1-5188-4002-985f-e8e0e77da127/"),
        dataType: "json",
        data: '[{"DFV":""},{"DMU":""}]'
    }).done(function (data) {
...
function play_dance(dance) {
    ...
    code = code | 4194304 | 512 ; //add the code that determines this is dance info.
